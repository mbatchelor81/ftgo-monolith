 ...
 *
 * <p>Restaurants are the curious case where a "builder" is really a builder-of-a-menu: a restaurant
 * without menu items is degenerate. The example below shows how to assemble a restaurant with one
 * and many menu items, and how to tap into {@link FtgoMothers#ajantaRestaurant()} for the canonical
 * "Ajanta" fixture carried over from the monolith regression suite.
 */
class RestaurantFixtureExampleTest {
  ...
  void aRestaurant_withMultipleMenuItems_retainsInsertionOrder() {
    ...
    assertThat(restaurant.menu()).extracting("id").containsExactly("vindaloo", "naan", "rice");
  }
  ...
  void ajantaRestaurant_exposesChickenVindalooPrice() {
    ...
    assertThat(restaurant.menu().get(0).price()).isEqualTo(FtgoMothers.CHICKEN_VINDALOO_PRICE);
  }
}
