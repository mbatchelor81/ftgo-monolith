...
/**
 * Integration tests verifying the full JWT authentication flow through the Spring Security filter
 * chain.
 *
 * <p>Tests that:
 *
 * <ul>
 *   <li>Requests without tokens receive 401
 *   <li>Requests with valid JWT tokens are authenticated
 *   <li>Invalid tokens are rejected with 401
 *   <li>User context (roles, userId) is available in controllers
 * </ul>
 */
@WebMvcTest
...
class JwtAuthenticationIntegrationTest {
    ...
    void requestWithoutToken_returns401() throws Exception {
        mockMvc.perform(get("/api/test/secure").accept(MediaType.APPLICATION_JSON))
    ...
    void requestWithValidAccessToken_returns200() throws Exception {
        String token =
                tokenService.generateAccessToken(
                        "user-1", "john.doe", List.of("USER"), List.of("order:read"));
    ...
    void requestWithValidToken_hasCorrectRoles() throws Exception {
        String token =
                tokenService.generateAccessToken(
                        "user-2", "admin.user", List.of("ADMIN", "USER"), List.of("order:write"));
    ...
    void requestWithExpiredToken_returns401() throws Exception {
        ...
        String validToken =
                tokenService.generateAccessToken("user-1", "test", List.of(), List.of());
        // Tamper with the token payload
        String tamperedToken =
                validToken.substring(0, validToken.lastIndexOf('.')) + ".invalidsignature";
