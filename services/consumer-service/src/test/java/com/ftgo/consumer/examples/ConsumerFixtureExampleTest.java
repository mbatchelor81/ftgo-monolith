 ...
 *   <ul>
 *     <li>{@link ConsumerBuilder} for fluent, minimal test-data setup.</li>
 *     <li>{@link FtgoMothers} object-mother constants that mirror the
 *         canonical fixtures used by the legacy monolith's Consumer tests
 *         so assertions can be kept stable during the migration.</li>
 *   </ul>
 *
 * <p>No Spring context is booted and no collaborators are needed; this is
 * the cheapest tier on the pyramid (see {@code docs/testing-strategy.md}
 * §2 and {@code docs/testing/when-to-write-which-test.md} §3).
 *
 * <p>Copy this class when writing the first real Consumer domain test:
 * replace the fixture assertions with assertions on the system under test
 * (e.g. {@code ConsumerService}, {@code CreateConsumerCommandHandler}).
 */
