 ...
 *
 * <ul>
 *   <li>{@link OrderBuilder} fluent API for assembling orders with deterministic totals (the
 *       builder auto-computes {@code orderTotal} from line items, matching the production {@code
 *       OrderLineItems.orderTotal()} semantics).
 *   <li>Domain-aware AssertJ assertions {@link com.ftgo.test.assertions.OrderAssert} and {@link
 *       com.ftgo.test.assertions.MoneyAssert} for readable failures.
 *   <li>{@link FtgoMothers} object mothers that mirror the legacy monolith's canonical fixtures —
 *       important so new service tests don't drift from long-standing regression suites.
 * </ul>
 *
 * <p>For a real order-service domain test see {@code
 * templates/test-templates/UnitTestTemplate.java}. This file's job is to prove the test-util
 * library works end-to-end on the order bounded context before the real {@code OrderService} lands
 * (EM-3x).
 */
class OrderFixtureExampleTest {
  ...
  void anOrder_withDefaults_matchesChickenVindalooFixture() {
    ...
    assertThat(order).hasState("APPROVED").hasLineItemCount(1).hasTotal(new Money("12.50"));
  }
  ...
  void chickenVindalooOrder_hasLegacyConsumerAndRestaurantIds() {
    ...
    assertThat(order).belongsToConsumer(FtgoMothers.CONSUMER_ID);
